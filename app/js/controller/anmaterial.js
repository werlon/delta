angular.module('primeiro')
    .controller('IndexControllerAnMaterial', IndexControllerAnMaterial);

IndexControllerAnMaterial.$inject = ['$scope','$timeout','AlertService','$filter'];
function IndexControllerAnMaterial($scope,$timeout,AlertService,$filter){
    ...
    function salvar(){
        ...
        if($scope.formPessoa.nascimento.$invalid === true){
            ...
            console.log('Data Nascimento inválida');
            return;
        }

        if($scope.formPessoa.cor.$invalid === true){
            AlertService.showWarning("Data","Erro");
